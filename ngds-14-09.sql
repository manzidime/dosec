...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `v_all_taxation` AS select `taxation`.`id_taxation` AS `id_taxation`,`taxation`.`id_exercice` AS `id_exercice`,`taxation`.`id_taxe` AS `id_taxe`,`taxation`.`id_site` AS `id_site`,`taxation`.`devise` AS `devise`,`taxation`.`num_taxation` AS `num_taxation`,`taxation`.`date_taxation` AS `date_taxation`,`taxation`.`taux` AS `taux`,`taxation`.`id_agent` AS `id_agent`,`taxation`.`date_creation` AS `date_creation`,`taxation`.`active` AS `active`,`taxation`.`state` AS `state`,`taxation`.`id_compte` AS `id_compte`,`taxation`.`date_validation` AS `date_validation`,`taxation`.`id_validateur` AS `id_validateur`,`ngds`.`taxation`.`nom_receptionniste` AS `nom_receptionniste`,`ngds`.`taxation`.`telephone_receptioniste` AS `telephone_receptioniste`,`ngds`.`exercice`.`annee` AS `exercice`,`ngds`.`site`.`lieu` AS `lieu`,`taxateur`.`nom` AS `nom_taxateur`,`taxateur`.`prenom` AS `prenom_taxateur`,`validateur`.`nom` AS `nom_validateur`,`validateur`.`prenom` AS `prenom_validateur`,`ngds`.`taxation`.`id_contribuable` AS `id_contribuable`,`ngds`.`contribuable`.`nom` AS `redevable`,`ngds`.`taxation`.`montant` AS `montant`,`ngds`.`taxation`.`id_vehicule` AS `id_vehicule`,`ngds`.`vehicule`.`numero_chassis` AS `numero_chassis`,`ngds`.`vehicule`.`numero_plaque` AS `numero_plaque`,`ngds`.`vehicule`.`model` AS `model`,`ngds`.`vehicule`.`marque` AS `marque`,`ngds`.`vehicule`.`couleur` AS `couleur`,`ngds`.`taxe`.`description` AS `description`,`ngds`.`taxation`.`penalite` AS `penalite`,`ngds`.`taxation`.`montant_global` AS `montant_global`,`ngds`.`taxe`.`designation` AS `taxe` from (((((((`taxation` join `exercice` on(`ngds`.`taxation`.`id_exercice` = `ngds`.`exercice`.`id_exercice`)) join `taxe` on(`ngds`.`taxation`.`id_taxe` = `ngds`.`taxe`.`id_taxe`)) join `site` on(`ngds`.`taxation`.`id_site` = `ngds`.`site`.`id`)) left join `agent` `taxateur` on(`ngds`.`taxation`.`id_agent` = `taxateur`.`id_agent`)) left join `agent` `validateur` on(`ngds`.`taxation`.`id_validateur` = `validateur`.`id_agent`)) join `contribuable` on(`ngds`.`taxation`.`id_contribuable` = `ngds`.`contribuable`.`id_contribuable`)) join `vehicule` on(`ngds`.`taxation`.`id_vehicule` = `ngds`.`vehicule`.`id_vehicule`));
